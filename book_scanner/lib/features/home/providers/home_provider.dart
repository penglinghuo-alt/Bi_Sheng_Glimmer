...
class HomeNotifier extends StateNotifier<HomeState> {
  ...
  void _playEntries(List<_LogEntry> entries, {void Function()? onDone}) {
    ...
    for (final entry in entries) {
      ...
      Future.delayed(Duration(milliseconds: delay), () {
        if (!mounted) return;
        if (entry.message.isNotEmpty) _log(entry.message);
      });
    }
    if (onDone != null) {
      Future.delayed(Duration(milliseconds: delay), () {
  ...
  void _phasePaperDetect() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][scanner] 纸张传感器触发，检测到纸张', 800),
      _LogEntry('[$_ts][INFO][scanner] 纸张厚度: ${thickness}mm, 材质: 盲文专用纸', 500),
    ];
  ...
  void _phaseTurnPage() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][scanner] 开始翻页 (第 $_currentPage/$_totalPages 页)...', 1000),
      _LogEntry('[$_ts][INFO][scanner] 翻页完成，步进电机到位，耗时 ${turnTime}s', 2500),
    ];
  ...
  void _phaseCapture() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][camera] 摄像头初始化...', 600),
      _LogEntry('[$_ts][INFO][camera] 正在拍摄第 $_currentPage 页图像...', 2000),
      _LogEntry('[$_ts][INFO][camera] 图像采集完成，分辨率 $res, 300DPI', 800),
      _LogEntry('[$_ts][INFO][camera] 图像预处理：去噪、二值化、倾斜校正', 2000),
      _LogEntry('[$_ts][INFO][camera] 预处理完成', 500),
      _LogEntry('', 2000),
    ];
  ...
  void _phaseOCR() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][ocr] 开始第 $_currentPage 页 OCR 文字识别...', 600),
      _LogEntry('[$_ts][INFO][ocr] 加载识别模型: chinese_ocr_v3.pth', 2000),
      _LogEntry('[$_ts][INFO][ocr] 模型加载完成，推理引擎: ONNX Runtime', 600),
      _LogEntry('[$_ts][INFO][ocr] 文字区域检测中...', 1500),
      _LogEntry('[$_ts][INFO][ocr] 检测到 $regions 个文字区域', 500),
      _LogEntry('[$_ts][INFO][ocr] 逐区域字符识别中...', 2500),
      _LogEntry('[$_ts][INFO][ocr] 区域 1-${regions ~/ 3} 识别完成 (进度: 33.3%)', 1500),
      _LogEntry('[$_ts][INFO][ocr] 区域 ${regions ~/ 3 + 1}-${regions * 2 ~/ 3} 识别完成 (进度: 66.7%)', 1500),
      _LogEntry('[$_ts][INFO][ocr] 区域 ${regions * 2 ~/ 3 + 1}-$regions 识别完成 (进度: 100.0%)', 1000),
      _LogEntry('[$_ts][INFO][ocr] 第 $_currentPage 页 OCR 识别完成，共检测 $charCount 个字符', 800),
      _LogEntry('[$_ts][INFO][ocr] 识别置信度: ${confidence.toStringAsFixed(1)}%', 500),
      _LogEntry('', 2500),
    ];
  ...
  void _phaseConvert() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][converter] 开始盲文点阵转换 (第 $_currentPage 页)...', 600),
      _LogEntry('[$_ts][INFO][converter] 加载盲文对照表: braille_table_v2.json', 1500),
      _LogEntry('[$_ts][INFO][converter] 文本分段处理中...', 800),
      _LogEntry('[$_ts][INFO][converter] 共 3 个段落，逐段转换', 500),
      _LogEntry('[$_ts][INFO][converter] 第 1 段落转换完成 ($c1 字符)', 1500),
      _LogEntry('[$_ts][INFO][converter] 第 2 段落转换完成 ($c2 字符)', 1500),
      _LogEntry('[$_ts][INFO][converter] 第 3 段落转换完成 ($c3 字符)', 1500),
      _LogEntry('[$_ts][INFO][converter] 点阵映射完成: ${w}x$h', 600),
      _LogEntry('[$_ts][INFO][converter] 盲文转换完成，开始传输数据', 1000),
      _LogEntry('', 2000),
    ];
  ...
  void _phasePrint() {
    ...
    final entries = [
      _LogEntry('[$_ts][INFO][printer] 打印头预热中...', 600),
      _LogEntry('[$_ts][INFO][printer] 打印头温度: ${temp1}°C / 目标 80.0°C', 1500),
      _LogEntry('[$_ts][INFO][printer] 打印头温度: ${temp2}°C / 目标 80.0°C', 1500),
      _LogEntry('[$_ts][INFO][printer] 打印头温度: ${temp3}°C，预热完成', 1000),
      _LogEntry('[$_ts][INFO][printer] 开始打印第 $_currentPage 页，总行数: $totalRows', 600),
      _LogEntry('[$_ts][INFO][printer] 打印行 1-${totalRows ~/ 5} (进度: 20.0%)', 1200),
      _LogEntry('[$_ts][INFO][printer] 打印行 ${totalRows ~/ 5 + 1}-${totalRows * 2 ~/ 5} (进度: 40.0%)', 1200),
      _LogEntry('[$_ts][INFO][printer] 打印行 ${totalRows * 2 ~/ 5 + 1}-${totalRows * 3 ~/ 5} (进度: 60.0%)', 1200),
      _LogEntry('[$_ts][INFO][printer] 打印行 ${totalRows * 3 ~/ 5 + 1}-${totalRows * 4 ~/ 5} (进度: 80.0%)', 1200),
      _LogEntry('[$_ts][INFO][printer] 打印行 ${totalRows * 4 ~/ 5 + 1}-$totalRows (进度: 100.0%)', 1200),
      _LogEntry('[$_ts][INFO][printer] 第 $_currentPage 页打印完成', 800),
    ];
  ...
  void _phasePaperAdvance() {
    state = state.copyWith(currentStep: PrintStep.turningPage);
    final entries = [
      _LogEntry('[$_ts][INFO][scanner] 纸张传送中...', 1500),
      _LogEntry('[$_ts][INFO][scanner] 机械臂归位', 1000),
    ];
    _playEntries(entries, onDone: () {
  ...
  void _phaseFinished() {
    final entries = [
      _LogEntry('[$_ts][INFO][scanner] 机械臂归位', 1000),
      _LogEntry('[$_ts][INFO][main] --- 本次打印任务全部完成，共 $_totalPages 页 ---', 1000),
    ];
    _playEntries(entries, onDone: () {
  ...
  void confirmReady() {
    final pageTimeMs = 3500 + 7900 + 16500 + 10500 + 14400 + 4500; // turn+capture+ocr+convert+print+advance
    _estimatedWorkMs = _totalPages * pageTimeMs - 4500 + 3000; // subtract last advance, add finish
